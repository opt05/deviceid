...
class NetworkRepository(private val context: Context, filterUnavailable: Boolean = false) :
    ...
    private fun deviceSoftwareVersion() =
        Item(R.string.network_title_device_software_version, ItemType.NETWORK).apply {
            try {
                subtitle =
    ...
    private fun wifiFrequency() =
        Item(R.string.network_title_wifi_frequency, ItemType.NETWORK).apply {
    ...
    private fun wifiHiddenSSID() =
        Item(R.string.network_title_wifi_hidden_ssid, ItemType.NETWORK).apply {
            try {
    ...
    private fun wifiIpAddress() =
        Item(R.string.network_title_wifi_ip_address, ItemType.NETWORK).apply {
            try {
                var ipAddress = wifiConnectionInfo.ipAddress
                if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) ipAddress =
    ...
    private fun wifiLinkSpeed() =
        Item(R.string.network_title_wifi_link_speed, ItemType.NETWORK).apply {
            try {
                subtitle = ItemSubtitle.Text(
    ...
    private fun wifiTxLinkSpeed() =
        Item(R.string.network_title_wifi_tx_link_speed, ItemType.NETWORK).apply {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                try {
                    subtitle = ItemSubtitle.Text(
                        wifiConnectionInfo.txLinkSpeedMbps.toString().plus(
    ...
    private fun wifiNetworkID() =
        Item(R.string.network_title_wifi_network_id, ItemType.NETWORK).apply {
            try {
    ...
    private fun wifiPasspointFqdn() =
        Item(R.string.network_title_wifi_passpoint_fqdn, ItemType.NETWORK).apply {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                try {
    ...
    private fun wifiPasspointProviderFriendlyName() =
        Item(R.string.network_title_wifi_passpoint_friendly_name, ItemType.NETWORK).apply {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                try {
    ...
    private fun wifiSignalLevel() =
        Item(R.string.network_title_wifi_signal_level, ItemType.NETWORK).apply {
            try {
                when {
                    Build.VERSION.SDK_INT >= Build.VERSION_CODES.R -> {
                        subtitle = ItemSubtitle.Text(
                            context.wifiManager
                                .calculateSignalLevel(wifiConnectionInfo.rssi).toString().plus("%")
                        )
                    }
                    else -> {
                        @Suppress("DEPRECATION")
                        subtitle = ItemSubtitle.Text(
                            WifiManager.calculateSignalLevel(wifiConnectionInfo.rssi, 100)
                                .toString().plus("%")
                        )
                    }
                }
    ...
    private fun wifiHostname() =
        Item(R.string.network_title_wifi_hostname, ItemType.NETWORK).apply {
            try {
     ...
     */
    @SuppressLint("HardwareIds")
    private fun bluetoothMac() =
        Item(R.string.network_title_bluetooth_mac, ItemType.NETWORK).apply {
            try {
    ...
    private fun bluetoothHostname() =
        Item(R.string.network_title_bluetooth_hostname, ItemType.NETWORK).apply {
            try {
                subtitle = ItemSubtitle.Text(
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
    ...
    private fun manufacturerCode() =
        Item(R.string.network_title_manufacturer_code, ItemType.NETWORK).apply {
            try {
                subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q)
                    ItemSubtitle.Text(context.telephonyManager.manufacturerCode)
    ...
    private fun phoneCount() = Item(R.string.network_title_phone_count, ItemType.NETWORK).apply {
        try {
            subtitle = when {
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.R -> {
                    ItemSubtitle.Text(context.telephonyManager.activeModemCount.toString())
                }
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.M -> {
                    @Suppress("DEPRECATION")
                    ItemSubtitle.Text(context.telephonyManager.phoneCount.toString())
                }
    ...
    private fun simSerial() = Item(R.string.network_title_sim_serial, ItemType.NETWORK).apply {
        try {
            subtitle = if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q)
                ItemSubtitle.Text(context.telephonyManager.simSerialNumber)
    ...
    private fun simOperatorName() =
        Item(R.string.network_title_sim_operator, ItemType.NETWORK).apply {
            try {
    ...
    private fun simState() = Item(R.string.network_title_sim_state, ItemType.NETWORK).apply {
        try {
            subtitle = ItemSubtitle.Text(
                when (context.telephonyManager.simState) {
    ...
    @SuppressLint("MissingPermission")
    private fun voicemailNumber() =
        Item(R.string.network_title_voicemail_number, ItemType.NETWORK).apply {
            try {
                subtitle =
                    if (context.isGranted(AppPermission.ReadPhoneState)) {
    ...
    private fun cellNetworkName() =
        Item(R.string.network_title_cell_network_name, ItemType.NETWORK).apply {
            try {
    ...
    @SuppressLint("MissingPermission")
    @Suppress("DEPRECATION")
    private fun cellNetworkType() =
        Item(R.string.network_title_cell_network_type, ItemType.NETWORK).apply {
            try {
                subtitle =
                    if (context.isGranted(AppPermission.ReadPhoneState)) {
                        ItemSubtitle.Text(
                            when (if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) context.telephonyManager.dataNetworkType else context.telephonyManager.networkType) {
    ...
    @SuppressLint("MissingPermission")
    @Suppress("DEPRECATION")
    private fun cellNetworkClass() =
        Item(R.string.network_title_cell_network_class, ItemType.NETWORK).apply {
            try {
                subtitle =
                    if (context.isGranted(AppPermission.ReadPhoneState)) {
                        ItemSubtitle.Text(
    ...
    private fun isConcurrentVoiceAndDataSupported() =
        Item(R.string.network_title_concurrent_voice_data_supported, ItemType.NETWORK).apply {
            try {
    ...
    private fun isDataRoamingEnabled() =
        Item(R.string.network_title_data_roaming_enabled, ItemType.NETWORK).apply {
            try {
                subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    if (context.isGranted(AppPermission.ReadPhoneState)) {
                    ...
                    ItemSubtitle.NotPossibleYet(Build.VERSION_CODES.Q)
                }
            } catch (e: Throwable) {
                Timber.w(e)
            }
    ...
    private fun isHearingAidSupported() =
        Item(R.string.network_title_hearing_aid_supported, ItemType.NETWORK).apply {
            try {
                subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ItemSubtitle.Text(context.telephonyManager.isSmsCapable.toString())
                } else {
    ...
    private fun isMultiSimSupported() =
        Item(R.string.network_title_multi_sim_supported, ItemType.NETWORK).apply {
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    subtitle =
                        if (context.isGranted(AppPermission.ReadPhoneState)) {
                            ItemSubtitle.Text(
                                when (context.telephonyManager.isMultiSimSupported) {
    ...
    private fun isRttSupported() =
        Item(R.string.network_title_rtt_supported, ItemType.NETWORK).apply {
            try {
                subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ItemSubtitle.Text(context.telephonyManager.isSmsCapable.toString())
                } else {
    ...
    private fun isSmsCapable() = Item(R.string.network_title_sms_capable, ItemType.NETWORK).apply {
        try {
            subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ItemSubtitle.Text(context.telephonyManager.isSmsCapable.toString())
            } else {
                ItemSubtitle.NotPossibleYet(Build.VERSION_CODES.LOLLIPOP)
            }
        } catch (e: Throwable) {
            Timber.w(e)
        }
    ...
    private fun isVoiceCapable() =
        Item(R.string.network_title_voice_capable, ItemType.NETWORK).apply {
            try {
                subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                    ItemSubtitle.Text(context.telephonyManager.isVoiceCapable.toString())
                } else {
                    ItemSubtitle.NotPossibleYet(Build.VERSION_CODES.LOLLIPOP_MR1)
                }
            } catch (e: Throwable) {
                Timber.w(e)
            }
    ...
    private fun eSimID() = Item(R.string.network_title_esim_id, ItemType.NETWORK).apply {
        try {
            subtitle = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                ItemSubtitle.Text(context.euiccManager.eid)
            } else {
                ItemSubtitle.NotPossibleYet(Build.VERSION_CODES.P)
            }
        } catch (e: Throwable) {
            Timber.w(e)
        }
    ...
    private fun eSimOSVersion() =
        Item(R.string.network_title_esim_os_version, ItemType.NETWORK).apply {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                try {
